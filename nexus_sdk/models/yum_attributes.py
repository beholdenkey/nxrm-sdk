...
class YumAttributes(object):
    ...
    def repodata_depth(self, repodata_depth):
        ...
        if repodata_depth is None:
            raise ValueError("Invalid value for `repodata_depth`, must not be `None`")  # noqa: E501
    ...
    def deploy_policy(self, deploy_policy):
        ...
        if deploy_policy not in allowed_values:
            raise ValueError(
                "Invalid value for `deploy_policy` ({0}), must be one of {1}"  # noqa: E501
                .format(deploy_policy, allowed_values)
            )
